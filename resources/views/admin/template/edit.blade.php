                        ...
                        <div class="w-full flex items-center justify-center">
                            <div class=" w-[400px] aspect-[2/1] max-h-full max-w-full rounded-md overflow-hidden shadow-md shadow-black/20 relative">
                                ...
                                <div class=" w-full relative">
                                    @php
                                        $headerImageSrc = $template->head_type === 'donuts' 
                                            ? 'donuts.png' 
                                            : ($template->head_type === 'ramen' ? 'one.jpg' : $template->head_type . '.jpg');
                                    @endphp
                                    <img id="header" src="{{asset('assets/images/template/header/'.$headerImageSrc)}}" class=" w-full duration-300 {{ $template->head_type === 'ramen' ? 'opacity-0' : '' }}" alt="">
                    ...
                    <div class=" w-full">
                        <div class=" w-full flex items-center justify-center">
                            <div x-data="{desctype: '{{$template->desc_type ?? 'default'}}'}"
                                ...
                                <div x-show="desctype === 'donuts'" id="desc-donuts-preview" class="w-full bg-white px-5 py-6 sm:px-6 relative" style="color: {{$template->desc_text_color ?? '#4A4A4A'}};">
                                    <div class="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2" style="border-color: {{$template->accent_color ?? '#E57384'}};"></div>
                                    <div class="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2" style="border-color: {{$template->accent_color ?? '#E57384'}};"></div>
                                    
                                    <div class="space-y-4 flex flex-col items-center text-center">
                                        <div class="min-w-0 flex-1 space-y-1">
                                            <p class="text-[10px] font-semibold uppercase tracking-[0.3em]" style="color: {{$template->accent_color ?? '#E57384'}};">Tentang Kami</p>
                                            <p class="text-lg font-serif italic text-[#2D2D2D] leading-tight">Nama Usaha</p>
                                        </div>
                                        <p class="text-xs leading-relaxed font-light opacity-80">Deskripsi usaha akan tampil dengan desain elegan dan estetik seperti ini.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class=" w-full">
                        <div class=" w-full flex items-center justify-center">
                            <div x-data="{producttype: '{{$template->product_type}}'}"
                                ...
                                <div x-show="producttype === 'donuts'" class="w-full space-y-3">
                                    <div class="flex flex-col items-center justify-center gap-1 text-center">
                                        <p class="text-[10px] font-semibold uppercase tracking-[0.2em]" style="color: {{$template->accent_color ?? '#E57384'}};">Koleksi Kami</p>
                                        <h2 class="text-lg font-serif italic text-[#2D2D2D]">Produk Pilihan</h2>
                                        <div class="w-6 h-[1px] mt-1" style="background-color: {{$template->accent_color ?? '#E57384'}};"></div>
                                    </div>
                                    <div class="grid grid-cols-2 gap-3 mt-3">
                                        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-md border-[#FFE4E8]">
                                            <div class="relative aspect-[4/5] bg-white">
                                                <img src="{{asset('assets/images/placeholder.webp')}}" class="w-full h-full object-cover" alt="">
                                                <div class="absolute top-2 left-2 px-1.5 py-0.5 rounded text-[8px] font-bold text-white shadow-sm bg-purple-500">BEST SELLER</div>
                                            </div>
                                            <div class="p-2 flex-1 flex flex-col items-center text-center">
                                                <p class="font-bold text-xs leading-tight mb-0.5 text-[#2D2D2D]">Produk 1</p>
                                                <p class="text-[10px] text-gray-400 line-clamp-1 flex-1 font-light">Deskripsi singkat produk</p>
                                                <p class="font-serif italic text-xs mb-2 mt-1" style="color: {{$template->accent_color ?? '#E57384'}};">Rp 20.000</p>
                                                <div class="w-full flex items-center gap-1 mt-auto">
                                                    <button type="button" class="flex-1 py-1 text-[10px] rounded-md font-medium text-center border" style="background-color: {{$template->product_second_color ?? '#FFF0F3'}}; color: {{$template->accent_color ?? '#E57384'}}; border-color: transparent;">Detail</button>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-md border-[#FFE4E8]">
                                            ...
                                            <div class="p-2 flex-1 flex flex-col items-center text-center">
                                                <p class="font-bold text-xs leading-tight mb-0.5 text-[#2D2D2D]">Produk 2</p>
                                                <p class="text-[10px] text-gray-400 line-clamp-1 flex-1 font-light">Deskripsi singkat produk</p>
                                                <p class="font-serif italic text-xs mb-2 mt-1" style="color: {{$template->accent_color ?? '#E57384'}};">Rp 24.000</p>
                                                <div class="w-full flex items-center gap-1 mt-auto">
                                                    <button type="button" class="flex-1 py-1 text-[10px] rounded-md font-medium text-center border" style="background-color: {{$template->product_second_color ?? '#FFF0F3'}}; color: {{$template->accent_color ?? '#E57384'}}; border-color: transparent;">Detail</button>
                                                </div>
                                            </div>
